// node_modules/@michaelhomer/jqjs/jq.js
...
function makeUserFuncFromNode(params, node, pathFunc = false) {
  ...
  ret.params = Array.prototype.map.call(params, (label) => ({
    label,
    mode: "defer",
  }));
  return ret;
...
function defineShorthandFunction(name, params, body) {
  ...
  functions[fname].params = Array.prototype.map.call(params, (label) => ({
    label,
    mode: "defer",
  }));
...
function tokenise(str, startAt = 0, parenDepth) {
  ...
  toplevel: for (i = startAt; i < str.length; i++) {
    ...
    if (c == '"' || c == "'") {
      ...
      if (isAlpha(d)) {
        i++;
        let tok = "";
        while (isAlpha(str[i]) || isDigit(str[i])) tok += str[i++];
  ...
  ret.push({
    type: "<end-of-program>",
    location: lineNum + ":" + (i - lineStart + 1),
  });
  return { tokens: ret, i };
}
    ...
    if (t.type == "identifier-index") {
      ...
      if (t.type == "as") {
        ...
        if (nameTok.type != "variable")
          throw (
            "expected variable name after as at " +
      ...
      if (tokens[i].type != "variable")
        throw (
          "expected variable name in reduce at " +
          describeLocation(tokens[i]) +
          " not " +
          tokens[i].type
        );
      ...
      if (tokens[i].type != "variable")
        throw (
          "expected variable name in foreach at " +
          describeLocation(tokens[i]) +
          " not " +
          tokens[i].type
        );
      ...
      if (tokens[i].type != "left-paren")
        throw (
          "expected left-paren in foreach at " + describeLocation(tokens[i])
        );
      r = parse(tokens, i + 1, ["semicolon"]);
      i = r.i;
      let init = r.node;
      ...
      if (
        !tokens[i] ||
        (tokens[i].type != "colon" && tokens[i].type != "left-paren")
      )
        throw (
          "expected : or parameter list when defining function " +
          nameTok.value +
          " at " +
          t.location
        );
      ...
      if (tokens[i].type == "left-paren") {
        ...
        while (tokens[i] && tokens[i].type != "right-paren") {
          if (tokens[i].type == "variable") {
            varParams.push({
              truename: "$inner" + params.length + "/0",
              bindname: tokens[i].name,
            });
            params.push("$inner" + params.length);
          } else {
            if (tokens[i].type != "identifier")
              throw (
                "expected identifier as parameter name in function " +
                name +
                " at " +
                tokens[i].location
              );
          ...
          } else if (tokens[i] && tokens[i].type == "semicolon") {
            i++;
          } else {
            throw (
              "expected ) or semicolon in parameter list of function " +
              name +
              " at " +
              tokens[i].location
            );
        ...
        if (!tokens[i] || tokens[i].type != "right-paren")
          throw (
            "expected ) to terminate parameter list of function " +
            name +
            " at " +
            nameTok.location
          );
      ...
      if (!tokens[i] || tokens[i].type != "colon")
        throw (
          "expected : when defining function " +
          nameTok.value +
          " at " +
          t.location
        );
      ...
      for (let vp of varParams) {
        let lhs = new VariableBinding(
          new FunctionCall(vp.truename, []),
          vp.bindname,
        );
      ...
      throw (
        ...
        " at " +
        describeLocation(t) +
        (until.length > 0 ? ", expected " + until.join("/") : "")
      );
    }
    t = tokens[++i];
  }
...
var FilterNode = class _FilterNode extends ParseNode {
  constructor(nodes) {
    super();
    ...
    if (p) {
      this.filter = p;
      this.source = nodes.length == 1 ? nodes[0] : new _FilterNode(nodes);
    }
  }
  *apply(input, conf) {
...
var IndexNode = class extends ParseNode {
    ...
    for (let l of this.lhs.apply(input, conf)) {
      ...
      for (let i of this.index.apply(input, conf)) {
        ...
        if (typeof i == "number" && i < 0 && nameType(l) == "array")
          yield l[l.length + i];
        else yield typeof l[i] == "undefined" ? null : l[i];
      }
    }
  }
  *paths(input, conf) {
...
var SliceNode = class extends ParseNode {
...
var GenericIndex = class extends ParseNode {
...
var IdentifierIndex = class extends GenericIndex {
  constructor(v) {
...
var GenericSlice = class extends ParseNode {
  constructor(fr, to) {
    super();
    this.from = fr;
    this.to = to;
...
var IdentityNode = class extends ParseNode {
  constructor() {
    super();
  }
  *apply(input, conf) {
...
var ValueNode = class extends ParseNode {
  constructor(v) {
    super();
    this.value = v;
  }
...
var StringNode = class extends ValueNode {
  constructor(v) {
    super(v);
  }
};
var StringLiteral = class extends ParseNode {
...
var NumberNode = class extends ValueNode {
  constructor(v) {
    super(v);
  }
  toString() {
...
var BooleanNode = class extends ValueNode {
  constructor(v) {
    super(v);
  }
  toString() {
...
var SpecificValueIterator = class extends ParseNode {
  constructor(source) {
    super();
    this.source = source;
...
var GenericValueIterator = class extends ParseNode {
  constructor() {
    super();
  }
  *apply(input, conf) {
...
var CommaNode = class extends ParseNode {
...
var ArrayNode = class extends ParseNode {
  constructor(body) {
    super();
    this.body = body;
  }
...
var PipeNode = class extends ParseNode {
  constructor(lhs, rhs) {
    super();
    this.lhs = lhs;
    this.rhs = rhs;
...
var ObjectNode = class extends ParseNode {
  constructor(fields) {
    super();
    this.fields = fields;
  }
...
var RecursiveDescent = class extends ParseNode {
  constructor() {
    super();
  }
  *apply(input, conf) {
...
var OperatorNode = class extends ParseNode {
  constructor(l, r) {
    super();
    this.l = l;
    this.r = r;
...
var AdditionOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var MultiplicationOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var SubtractionOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var DivisionOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var ModuloOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var LessThanOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var GreaterThanOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var LessEqualsOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var GreaterEqualsOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var AndOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var OrOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var EqualsOperator = class extends OperatorNode {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var NotEqualsOperator = class extends EqualsOperator {
  constructor(l, r) {
    super(l, r);
  }
  combine(l, r, lt, rt) {
...
var AlternativeOperator = class extends ParseNode {
  constructor(l, r) {
    super();
    this.lhs = l;
    this.rhs = r;
...
var UpdateAssignment = class extends ParseNode {
  constructor(l, r) {
    super();
    this.l = l;
    this.r = r;
...
var PlainAssignment = class extends ParseNode {
  constructor(l, r) {
    super();
    this.l = l;
    this.r = r;
...
var FunctionCall = class extends ParseNode {
  ...
  apply(input, conf) {
    ...
    if (ufa)
      func = function (input2, conf2, args) {
        return ufa.apply(input2, conf2);
      };
  ...
  toString() {
    if (this.args.length == 0) return this.name.replace(/\/.*$/, "");
    else
      return this.name.replace(/\/.*$/, "") + "(" + this.args.join("; ") + ")";
  }
};
var FormatNode = class extends ParseNode {
  ...
  *apply(input, conf) {
  ...
  toString() {
    if (this.string) return "@" + this.name + " " + this.string;
    else return "@" + this.name;
  }
};
var ErrorSuppression = class extends ParseNode {
  constructor(inner) {
    super();
    this.inner = inner;
  }
...
var VariableBinding = class extends ParseNode {
...
var VariableReference = class extends ParseNode {
  constructor(name) {
    super();
    this.name = name;
  }
...
var ReduceNode = class extends ParseNode {
...
var ForeachNode = class extends ParseNode {
  ...
  *apply(input, conf) {
    for (let accum of this.init.apply(input, conf)) {
      for (let v of this.generator.apply(input, conf)) {
        conf.variables[this.name] = v;
        for (let a of this.update.apply(accum, conf)) accum = a;
        for (let e of this.extract.apply(accum, conf)) yield e;
      }
      delete conf.variables[this.name];
    }
  }
  ...
  sh(v) {
    ...
    if (t == "string") return "'" + t.replace(/'/g, "'\\''") + "'";
      ...
      return v
        .map((v2) => {
          let t2 = nameType(v2);
          if (t2 == "string") return "'" + v2.replace(/'/g, "'\\''") + "'";
          else if (t2 == "number") return "" + v2;
          else if (t2 == "boolean") return "" + v2;
          else if (v2 === null) return "null";
          else throw t2 + " cannot be escaped for shell";
        })
        .join(" ");
...
var functions = {
  ...
  "path/1": Object.assign(
    function* (input, conf, args) {
      let f = args[0];
      yield* f.paths(input, conf);
    },
    { params: [{ mode: "defer" }] },
  ),
  "select/1": Object.assign(
    function* (input, conf, args) {
      let selector = args[0];
      for (let b of selector.apply(input, conf))
        if (b !== false && b !== null) yield input;
    },
    {
  ...
  "add/1": Object.assign(
    function* (input, conf, args) {
      ...
      for (let addend of args[0].apply(input, conf)) {
        sum = AdditionOperator.prototype.combine(
          sum,
          addend,
          nameType(sum),
          nameType(addend),
  ...
  "toboolean/0": function* (input) {
    if (input === "false" || input === false) yield false;
    else if (input === "true" || input === true) yield true;
    else
  ...
  "getpath/1": Object.assign(
    function* (input, conf, args) {
      if (nameType(input) != "object" && nameType(input) != "array")
        throw (
          "can only get path from objects and arrays, not " + nameType(input)
        );
  ...
  "setpath/2": Object.assign(
    function* (input, conf, args) {
      if (
        nameType(input) != "object" &&
        nameType(input) != "array" &&
        nameType(input) != "null"
      )
      ...
      for (let path of args[0].apply(input, conf)) {
        ...
        for (let key of path.slice(0, -1)) {
          if (obj === null) {
            ...
            if (nameType(key) == "number") current[key] = [];
            else current[key] = {};
          }
          current = current[key];
        }
        for (let val of args[1].apply(input, conf)) {
          ...
          if (obj === null) {
            ...
            if (nameType(key) == "number") current = [];
            else current = {};
          }
          current[key] = val;
        }
  ...
  "delpaths/1": Object.assign(
    function* (input, conf, args) {
      if (nameType(input) != "object" && nameType(input) != "array")
        throw (
          "can only delete paths from objects and arrays, not " +
          nameType(input)
        );
  ...
  "nth/1": Object.assign(
    function* (input, conf, args) {
      ...
      for (let n of args[0].apply(input, conf)) {
        ...
        if (n < 0) throw "negative indices not supported for nth";
        yield input[n];
      }
    },
    { params: [{ label: "index" }] },
  ...
  "isempty/1": function* (input, conf, args) {
    for (let item of args[0].apply(input, conf)) return yield false;
    return yield true;
  },
  "limit/2": Object.assign(
    function* (input, conf, args) {
      for (let n of args[0].apply(input, conf)) {
        let count = 0;
        for (let val of args[1].apply(input, conf)) {
    ...
    for (let re of args[0].apply(input, conf)) {
      for (let flags of args[1].apply(input, conf)) {
        ...
        while (null !== (match = regexp.exec(input))) {
          console.log(match);
          if (match.length > 1) yield match.slice(1);
          else yield match[0];
        }
      }
    }
  ...
  "endswith/1": function* (input, conf, args) {
    for (let v of args[0].apply(input, conf)) yield input.endsWith(v);
  },
  "startswith/1": function* (input, conf, args) {
    for (let v of args[0].apply(input, conf)) yield input.startsWith(v);
  },
  ...
  "max/0": function* (input) {
    let best = null;
    for (let o of input) {
      if (compareValues(o, best) > 0) best = o;
    }
    yield best;
  },
  "min/0": function* (input) {
    let best = void 0;
    for (let o of input) {
      if (typeof best === "undefined") best = o;
      else if (compareValues(o, best) < 0) best = o;
    }
        ...
        if (nameType(needle) == "array") {
          outer: for (let i = 0; i < input.length - needle.length; i++) {
            for (let j = 0; j < needle.length; j++)
              if (compareValues(input[i + j], needle[j]) != 0) continue outer;
            ret.push(i);
          }
        } else {
          for (let i = 0; i < input.length; i++) {
            if (compareValues(input[i], needle) == 0) ret.push(i);
          }
        }
        yield ret;
      }
  ...
  "flatten/1": function* (input, conf, args) {
    if (nameType(input) != "array")
      throw "can only flatten array, not " + nameType(input);
    for (let depth of args[0].apply(input, conf)) yield input.flat(depth);
  },
  "transpose/0": function* (input) {
    ...
    for (let y = 0; y < size; y++) {
      let row = [];
      ret.push(row);
      for (let x = 0; x < size; x++) row.push(input[x][y] ?? null);
    }
    yield ret;
  },
  "combinations/0": function* (input) {
    function* helper(sofar, i) {
      let arr = input[i];
      if (arr === void 0) return yield sofar;
      for (let a of arr) {
        let pfx = [...sofar, a];
  ...
  if (mathName in Math) {
    functions[mf + "/2"] = function* (input, conf, args) {
      for (let a1 of args[0].apply(input, conf))
        for (let a2 of args[1].apply(input, conf)) yield Math[mathName](a1, a2);
    };
  }
}
  ...
  for (let regexp of regexpExpr.apply(input, conf)) {
    ...
    for (let i = 0; i < bits.length; i += 2) {
      ...
      for (let replacement of replacementExpr.apply(groups, conf)) {
        these.push(before2 + replacement);
      }
    }
    let index = 0;
    while (true) {
      ...
      for (let i = 0; i < results.length; i++) {
        if (typeof results[i][index] !== "undefined") anyPresent = true;
        r += results[i][index] ?? "";
      }
      if (!anyPresent) break;
      r += finalPiece;
...
defineShorthandFunction(
  "scalars",
  "",
  'select(type != "array" and type != "object")',
);
defineShorthandFunction(
  "pick",
  ["pathexps"],
  ". as $in | reduce path(pathexps) as $a (null; setpath($a; $in|getpath($a)) )",
);
...
defineShorthandFunction(
  "combinations",
  "n",
  ". as $input | [ range(n) | $input ] | combinations",
);
...
defineShorthandFunction(
  "JOIN",
  ["idx", "stream", "idx_expr", "join_expr"],
  "idx as $idx | stream | idx_expr as $key | [ ., $idx[$key] ] | join_expr",
);
defineShorthandFunction(
  "JOIN",
  ["idx", "stream", "idx_expr"],
  "idx as $idx | stream | [., $idx[idx_expr]]",
);
defineShorthandFunction(
  "JOIN",
  ["idx", "idx_expr"],
  "idx as $idx | [.[] | [., $idx[idx_expr]]]",
);
...
function combined(prog, input, ...rest) {
  if (nameType(prog) == "array") {
    ...
    for (let i = 0; i < prog.length; i++) {
      ...
      if (rest[i] !== void 0) collected.push(JSON.stringify(rest[i]));
    }
...
var jq = Object.assign(combined, { compile, prettyPrint });
var jq_default = jq;
export { compile, compileNode, jq_default as default, formats, prettyPrint };
