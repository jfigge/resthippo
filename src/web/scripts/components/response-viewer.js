...
export class ResponseViewer {
  ...
  constructor() {
    ...
    window.addEventListener("wurl:request-loading", (e) => {
      ...
      if (isSelected(rid)) {
        ...
        this.#armedStreamId = e.detail?.streamId ?? null;
      }
    });
    ...
    window.addEventListener("wurl:stream-data", (e) =>
      this.#onStreamData(e.detail),
    );
    window.addEventListener("wurl:stream-end", (e) =>
      this.#onStreamEnd(e.detail),
    );
  ...
  async #showBodyContextMenu(x, y) {
    ...
    if (clickedId === "download") {
      ...
      if (this.#streaming) this.#saveStream();
  ...
  #showResponse(response, requestUrl) {
    ...
    if (response.streaming === true) {
      this.#startStream(response, requestUrl);
      return;
    }
  ...
  #startStream(response, requestUrl) {
    ...
    this.#streamBytes = 0;
    this.#streamEvents = 0;
    this.#lastResponse = null; // streaming has no static body to re-render
    this.#isHtmlResponse = false;
    this.#setPreviewTabVisible(false);

    if (response.request?.method)
      this.#setCurrentMethod(response.request.method);
    ...
    const badge = this.#statusBar.querySelector(".res-status-badge");
    badge.className = `res-status-badge ${this.#statusClass(response.status ?? 0)}`;
  ...
  #updateStreamCounts() {
    const text = t("response.stream.counts", {
      ...
      size: this.#formatSize(this.#streamBytes),
    });
    if (this.#streamCountsEl) this.#streamCountsEl.textContent = text;
    const sizeEl = this.#statusBar.querySelector(".res-size");
    if (sizeEl) sizeEl.textContent = this.#formatSize(this.#streamBytes);
  }
   ...
   */
  #onStreamHint(d) {
    if (!d || d.streamId !== this.#armedStreamId || !this.#loadingTimer) return;
    if (
      !this.#bodyPane ||
      this.#bodyPane.querySelector(".res-stream-hint-banner")
    )
      return;
    this.#bodyPane.insertBefore(
      this.#buildNdjsonStreamHint(),
      this.#bodyPane.firstChild,
    );
  }
  ...
  #finishStream(d, { aborted = false, error = null } = {}) {
    ...
    if (typeof d.elapsed === "number") {
      this.#statusBar.querySelector(".res-time").textContent =
        `${d.elapsed} ms`;
    }
    this.#statusBar.querySelector(".res-size").textContent = this.#formatSize(
      this.#streamBytes,
    );
  ...
  #renderStreamSummary(summary, pane) {
    ...
    addStat(t("response.streamRecord.bytes"), this.#formatSize(bytes));
