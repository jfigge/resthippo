...
const PRISM_LANG = {
  ...
  xml: "markup",
  graphql: "graphql",
  // Request scripting (Feature 25). Highlight only — JS validation is host-driven
  // (externalErrors: true → setErrors()), so `javascript` stays out of
  // VALIDATED_LANGS below.
...
export class PillCodeEditor {
  ...
  /** Re-render the current error squiggles against the live layout. Needed when
   *  the editor was validated while hidden — zero-size rects suppress the
   *  squiggle elements (see #renderErrorMarkers) — and has since become visible,
   *  e.g. when its host tab is first shown. */
  refreshMarkers() {
    this.#renderErrorMarkers();
  }
