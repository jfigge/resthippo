...
export async function refreshTokenFlow(config, refreshToken) {
  if (!refreshToken?.trim())          return oauthResultFromError(configurationError("No refresh token available."));
  if (!config.accessTokenUrl?.trim()) return oauthResultFromError(configurationError("Access Token URL is required."));

  const params  = { grant_type: "refresh_token", refresh_token: refreshToken.trim() };
  const headers = {};
