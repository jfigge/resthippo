...
/**
 * auth/flows/token-request.js
 *
 * Shared building blocks for the OAuth grants that POST to a token endpoint
 * (authorization-code, client-credentials, password, refresh-token, and the
 * RFC 8693 token-exchange grant). Three concerns recurred verbatim across those
 * flows and are centralised here:
 *
 ...
 *                          access_token guards).
 *
 * NOTE on the name: this module is the shared *token-POST helper*, not the
 * RFC 8693 token-exchange grant (that lives in flows/token-exchange.js). It was
 * historically misnamed `token-exchange.js`; the rename to `token-request.js`
 * makes the filename match its role.
 *
 ...
 * The device-authorization grant polls the token endpoint with non-terminal
 * 4xx bodies (`authorization_pending` / `slow_down`), so it drives its own loop
 * in flows/device-code.js rather than reusing requestToken().
 */
