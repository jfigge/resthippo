...
dropdown and fill in the fields — Rest Hippo applies the right header (or query
...
**Header** or the **Query Param**. Rest Hippo suggests common key names as you type.
...
For **Digest**, supply a username and password; Rest Hippo performs the
...
**Secret Access Key**, **Region**, and **Service** (Rest Hippo can infer the service
...
3. Click **Get Token**. For browser-based flows (Authorization Code, Implicit),
   Rest Hippo opens a popup window for you to log in, intercepts the redirect, and
